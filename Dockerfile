...
# install s6 overlay
ARG TARGETARCH
RUN if [ "$TARGETARCH" = "amd64" ]; then ARCHITECTURE=x86_64; elif [ "$TARGETARCH" = "arm/v7" ]; then ARCHITECTURE=arm; elif [ "$TARGETARCH" = "arm/v6" ]; then ARCHITECTURE=armhf; elif [ "$TARGETARCH" = "arm64" ]; then ARCHITECTURE=aarch64; else ARCHITECTURE=x86_64; fi \
